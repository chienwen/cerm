<?php
    ...
    class JCBExchangeRateFetcher extends ExchangeRateFetcher {
        ...
        private function doFetch($url){
            $data = curl($url);
            foreach(explode("\n",$data) as $line){
                $tline = trim($line);
                $toks = explode(',', $tline);
                if(count($toks) >= 6 && preg_match('/^USD,=,/',$line)){
                    //print_r($toks);
                    $this->extable[trim($toks[5])] = array(
                                'mid' => trim($toks[3])/1,
                                'buy' => trim($toks[2])/1,
                                'sell' => trim($toks[4])/1,
                            );
        ...
        protected function fetch(){
            ...
            $this->doFetch('http://www.jcb.jp/uploads/'.date('Ymd', $this->ts).'.csv');

            // if fail, re-fetch the pattern
            if(empty($this->extable)){
                $csvfile = false;
                $data = curl('http://www.jcb.jp/rate/usd'.date('mdY', $this->ts).'.html');
                foreach(explode("\n",$data) as $line){
                    if(preg_match('/CSVToTable\(\'\/uploads\/(.+\.csv)\',/',$line,$regs)){
                        $csvfile = $regs[1];
                    }
                }
                if(!empty($csvfile)){
                    $this->doFetch('http://www.jcb.jp/uploads/'.$csvfile);
                }
            }
        }
    }
